...
(* Why3 assumption *)
Definition update0 (self:t_unionfindmodel) (i:uint64) (r:uint64) :
    t_unionfindmodel :=
...
Axiom lemma_update_in_range0_spec :
  forall (self:t_unionfindmodel), forall (j:Numbers.BinNums.Z),
  forall (r:uint64), forall (i:uint64), in_range0 self (uint64'int i) ->
  in_range0 (update0 self i r) j <-> in_range0 self j.

Axiom lemma_update_parent_ne0_spec :
  forall (self:t_unionfindmodel), forall (j:Numbers.BinNums.Z),
  forall (r:uint64), forall (i:uint64), in_range0 self (uint64'int i) ->
  in_range0 self j -> ~ ((uint64'int i) = j) ->
  ((parent0 (update0 self i r) j) = (parent0 self j)).

Axiom lemma_update_parent_eq0_spec :
  forall (self:t_unionfindmodel), forall (r:uint64), forall (i:uint64),
  in_range0 self (uint64'int i) ->
  ((parent0 (update0 self i r) (uint64'int i)) = (uint64'int r)).
...
Parameter lemma_update_belong_preserved:
  t_unionfindmodel -> uint64 -> uint64 -> Init.Datatypes.unit.

(* Why3 goal *)
Theorem vc_lemma_update_belong_preserved :
  is_root0 self (uint64'int r) -> in_range0 self (uint64'int r) ->
  in_range0 self (uint64'int i) -> invariant0 self ->
  (forall (r1:uint64), forall (i1:uint64), in_range0 self (uint64'int i1) ->
   ((parent0 (update0 self i1 r1) (uint64'int i1)) = (uint64'int r1))) ->
  (forall (j:Numbers.BinNums.Z), forall (r1:uint64), forall (i1:uint64),
   in_range0 self (uint64'int i1) -> in_range0 self j ->
   ~ ((uint64'int i1) = j) ->
   ((parent0 (update0 self i1 r1) j) = (parent0 self j))) ->
  (forall (j:Numbers.BinNums.Z), forall (r1:uint64), forall (i1:uint64),
   in_range0 self (uint64'int i1) ->
   in_range0 (update0 self i1 r1) j <-> in_range0 self j) ->
  (forall (src:Numbers.BinNums.Z), reach0 self src src 0%Z) /\
  (forall (len:Numbers.BinNums.Z), forall (dst:Numbers.BinNums.Z),
   forall (src:Numbers.BinNums.Z), ~ (src = dst) ->
   reach0 self (parent0 self src) dst len ->
  ...
  in_range0 self j ->
  ~ (exists n:Numbers.BinNums.Z, reach0 self j (uint64'int i) n) ->
  belong0 (update0 self i r) j g <-> belong0 self j g.
Proof.
  Open Scope Z_scope.
  intros Hisroot_r Hinrange_r Hinrange_i Hself_inv _ _ _ _ _ _ g j Hinrange_j Hunreach.

  pose proof (lemma_update_parent_eq0_spec self r i Hinrange_i) as update_parent_eq.
  pose proof (fun j p1 p2 => lemma_update_parent_ne0_spec self j r i Hinrange_i p1 p2)
    as update_parent_ne.
  pose proof (fun j => lemma_update_in_range0_spec self j r i Hinrange_i)
    as update_in_range.
  set (i' := uint64'int i).
  remember (update0 self i r) as updated eqn:e; clear e.
  destruct (Z.eq_dec i' g) as [Heq_ig  | Hne_ig].
  - rewrite <- Heq_ig in *. clear Heq_ig.
    assert (~belong0 self j i') as Hnotbelong by (unfold belong0; tauto).
    enough (~belong0 updated j i') by tauto. clear Hnotbelong.
    ...
    rewrite lemma_reach_iff0_spec in Hreach.
    destruct Hreach as [| (Hne & Hpos & Hreach)].
    + apply Hunreach. exists 0. rewrite lemma_reach_iff0_spec. tauto.
    + rewrite update_parent_ne in Hreach; auto.
      apply IH in Hreach; auto with zarith.
      * intros [len' Hreach']. apply Hunreach. exists (len' + 1).
        rewrite lemma_reach_iff0_spec. right.
        pose proof (lemma_reach_nonneg0_spec _ _ _ _ Hreach').
        repeat split; auto with zarith.
        replace (len' + 1 - 1) with len'; auto with zarith.
      * now apply Hself_inv.
  - unfold belong0.
    cut ((exists len, reach0 updated j g len) <-> (exists len, reach0 self j g len)). {
      enough (is_root0 updated g <-> is_root0 self g) by tauto.
      unfold is_root0. rewrite update_in_range.
      split; intros [Hin Hparent]; rewrite (update_parent_ne _ Hin) in *; auto.
    }
    cut (forall len, reach0 updated j g len <-> reach0 self j g len). { clear. firstorder. }
    intro len.
    revert j Hinrange_j Hunreach.
    induction len as [len IH] using (well_founded_induction (Z.lt_wf 0)).
    intros j Hinrange_j Hunreach.
    rewrite (lemma_reach_iff0_spec updated).
    rewrite (lemma_reach_iff0_spec self).
    ...
    + rewrite update_parent_ne; auto.
      * rewrite IH; try reflexivity; auto with zarith.
        ...
        -- intros [len' Hreach].
           ...
           ++ exists 0. rewrite lemma_reach_iff0_spec. auto.
           ++ exists (len' + 1).
              rewrite lemma_reach_iff0_spec. right.
              replace (len' + 1 - 1) with len'; auto with zarith.
              repeat split; auto with zarith.
              apply lemma_reach_nonneg0_spec in Hreach.
              auto with zarith.
      * intro. subst. apply Hunreach. exists 0. rewrite lemma_reach_iff0_spec. tauto.
    + cut (forall s i j, ~reach0 s i j (len - 1)). { clear. firstorder. }
      intros. rewrite lemma_reach_iff0_spec. auto with zarith.
Qed.

(* Unused content named lemma_update_belong_preserved'vc
Proof.
